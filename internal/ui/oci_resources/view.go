		...
		Sections: []help.Section{
			{
				...
				Body: "Shows all local " + engTitle + " images with disk usage, content size, and CVE scan results.\n" +
					"Update: an arrow when the image's registry has something newer — a later patch tag on the same line (node:20.11.1 → 20.11.4), or new content behind the same tag (\"new build\": latest, a codename, or a tag rebuilt in place). Only pulled images are asked; the registries are asked in the background when the list loads, and each answer is kept 6 hours. Pull the image (G) and the arrow goes away at once. Blank means nothing newer is known — up to date, not checked yet, or an image built here.\n" +
